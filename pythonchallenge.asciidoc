...
Go to check the cookies of http://www.pythonchallenge.com/pc/def/linkedlist.php[the page of Level 4], this can be done by *cookielib* or devbug console of *firefox* or *chromium*.
...
Collect all the cookies, yeild a string begins with *"BZh91AY%26SY"*, check the magic header of a typical _.bz2_ file:

[source, bash]
$ head -c 10 test.bz2 # yeild "BZh91AY&SY"

So we need to decompress this string with bzip2, but first it need to be unquoted to change *%26* to *&* (http://www.w3schools.com/tags/ref_urlencode.asp[HTML URL Decoding]). This can be done by *urllib.unquote*.
...
=== http://www.pythonchallenge.com/pc/stuff/violin.php[18]
